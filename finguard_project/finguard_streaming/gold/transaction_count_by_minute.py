...
,comment="Alert details where transactions has been peformed with value higher than what is configured by customer"
)
...
def transaction_count_by_minute() -> DataFrame:
    ...
    transaction_count_df = transactions_with_watermark.groupBy(
        F.window("transaction_timestamp", "1 minutes")
    ).agg(
